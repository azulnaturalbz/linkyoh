...
class ControllerTests(unittest.TestCase):
    ...
    def test_v2_readiness_requires_version_and_games_link(self):
        class Response:
            status = 200
        ...
        for body, ready in ((b'linkyoh-stitch-wordmark', False),
                            (b'data-strip-version="v2"', False),
                            (b'data-strip-version="v2" data-track="games_clickout"', True)):
            response = Response()
            response.body = body
            with self.subTest(body=body), patch.object(controller.urllib.request, 'urlopen', return_value=response), patch.object(controller.time, 'sleep'):
                if ready:
                    controller.ready()
                else:
