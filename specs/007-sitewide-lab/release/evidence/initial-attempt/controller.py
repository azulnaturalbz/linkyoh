...
RELEASE = ROOT / 'releases/34196aa-20260914'
OLD = 'sha256:e252a3ce5593b3f43da9f84b445e43afbcd1b589fa225272ca839d8f7995d9c0'
...
def snapshot(name):
    ...
    data = {'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            'caddy_peer': peer, 'web_aliases': sorted(web['NetworkSettings']['Networks']['edge']['Aliases']),
...
def prepare():
    ...
    run('docker', 'tag', OLD, 'linkyoh-web:rollback-pre-34196aa')
    print(json.dumps({'state': 'BACKED_UP', 'files': len(manifest['source']), 'checksums': checksums}))


def ready(new_ui=True):
    for _ in range(45):
        try:
            with urllib.request.urlopen('https://linkyoh.com/', timeout=8) as response:
                body = response.read()
                if response.status == 200 and (not new_ui or b'linkyoh-stitch-wordmark' in body):
                    return
        except Exception:
            pass
        time.sleep(2)
