...
try{
  ...
  for(const asset of ['static/lab/sitewide.css','static/lab/discovery.css','static/lab/assets/linkyoh-stitch-wordmark.png','static/lab/assets/linkyoh-belize-banner.png','static/brand/kev/kev-plumbing-master.png']){
  ...
  for(const route of ['/robots.txt','/llms.txt','/sitemap.xml']){const r=await fetch(base+route),body=await r.text();report.https.push({url:base+route,status:r.status,bytes:body.length});assert(r.status===200&&body.length>100,'crawl surface');}
