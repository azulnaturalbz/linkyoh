...
if [ -z "$DOMAIN_NAME" ]; then
  DOMAIN_NAME="linkyoh.com"
fi

# Set domains array with main domain and www subdomain
domains=($DOMAIN_NAME www.$DOMAIN_NAME)
rsa_key_size=4096
data_path="./nginx/certbot"
email=${EMAIL:-"dev@silvatech.org"} # Adding a valid address is strongly recommended
...
echo "### Starting nginx ..."
docker-compose -f docker-compose.standalone.yml up --force-recreate -d nginx
echo
